...
FlyingEntity::FlyingEntity(std::shared_ptr<Entity> entity, int flyRate) : EntityDecorator(std::move(entity)), flyRate(flyRate) {
}

std::shared_ptr<Entity> FlyingEntity::clone() {
    auto e = std::make_shared<FlyingEntity>(EntityDecorator::clone(), flyRate);
    return std::move(e);
}
